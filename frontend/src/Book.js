...
function Book(props) {
		...
		if (!getFlightsDebounce) {
			...
			try {
				const resp = await axios.get(
					`http://localhost:5000/api/flights?from=${departingAirport.name}&to=${arrivingAirport.name}`,
					{
						headers: {
	...
	const bookFlight = async () => {
		if (!bookDebounce) {
			...
			try {
				const resp = await axios.get(
					`http://localhost:5000/api/flights?from=${selectedFlight.name}&to=${arrivingAirport.name}`,
					{
						headers: {
							authorization: `Bearer ${loginInfo.token}`,
						},
			...
			<DialogBox
				...
				<form onSubmit={handleBookSubmit}>
					<div className="mt-4">
						...
						{bookFlightError && (
							<p className="text-red-500"> {bookFlightError} </p>
						)}
					</div>
					<div className="mt-4 space-x-2">
						<button
							...
							onClick={() => bookFlight()}
							className={`${
								bookDebounce ? "cursor-wait" : "cursor-pointer"
